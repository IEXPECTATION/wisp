...
export class Parser {
  ...
  private blockQuote(tokens: Tokens, scanner: Scanner) {
    ...
    while (!scanner.Eos()) {
      ...
      // TODO: Push a EOL back to buffer
      buffer += EOL;

      let count = this.whiteSpace(scanner);
      if (count >= TAB_SIZE) {
        scanner.Retreat(count);
        break;
      }

      // Find a blank line inside a block quote or outside it.
      scanner.Consume(">");
    ...
    if (buffer == '') {
      tokens.push({ Kind: Element.BlockQuote, Tokens: [] } as BlockQuoteToken)
      return true;
    }

    let subTokens = this.parse(new Scanner(buffer));
    if (subTokens.length == 0) {
      scanner.FlashBack();
      return false;
    }

    // Access the last token is a paragraph or not.
    let token = subTokens[subTokens.length - 1];
    while (true) {
      if (token.Kind == Element.BlockQuote && (token as BlockQuoteToken).Tokens.length > 0) {
        token = (token as BlockQuoteToken).Tokens[(token as BlockQuoteToken).Tokens.length - 1];
        continue;
      }
      break;
    }

    // Find the continuation text following the first line of paragraph.
    if (token?.Kind == Element.Paragraph) {
      ...
      while (!scanner.Eos()) {
        ...
        // TODO: The list element can also interrept the paragraph.

        while (!this.eol(scanner)) {
          successor += scanner.Peek();
          scanner.Advance();
        }

        (token as ParagraphToken).Text += successor;
      }
    }


    tokens.push({ Kind: Element.BlockQuote, Tokens: subTokens } as BlockQuoteToken);
    return true;
  }
  ...
  private isSetextHeading(scanner: Scanner): number {
  ...
  private paragraph(tokens: Tokens, scanner: Scanner) {
    ...
    while (!scanner.Eos()) {
      ...
      // Check next line is still a paragraph
      if (this.blankline(nextTokens, scanner)) {
        break;
      }
      ...
      let level = this.isSetextHeading(scanner);
      if (level != 0) {
        tokens.push({ Kind: Element.Heading, Text: text, Level: level } as HeadingToken);
        return true;
      }
